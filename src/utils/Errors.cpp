...
Error::Error(std::string name, std::string message, std::size_t line_no,
...
SyntaxError::SyntaxError(std::string message, std::size_t line_no,
						 std::string line_content)
...
ZeroDivisionError::ZeroDivisionError(std::size_t line_no,
									 std::string line_content)
	: Error("ZeroDivisionError",
...
TypeError::TypeError(std::string t1, std::string t2, std::size_t line_no,
					 std::string line_content)
	: Error("TypeError", "Cannot convert from " + t1 + " to " + t2, line_no,
			line_content) {}

std::ostream &operator<<(std::ostream &os, const Error *error) {
	system((std::string("echo '\033[1;31m") + error->name_ +
